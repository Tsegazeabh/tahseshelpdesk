<template>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 dark:text-gray-500">Privacy Policy</span>
                </div>
            </li>
        </ol>
    </nav>
    <div class="flex items-center justify-between">
        ...
        <router-link  :to="{name:'create_privacy_policy'}" class="text-sm float-right p-2 bg-green-400 text-white hover:bg-green-500 rounded-md">Create</router-link>
    </div>
    ...
    <div class="flex flex-col mt-6 w-full">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <template v-if="all_privacy_policy && all_privacy_policy.length !== 0 && !isLoading">
                    <tr v-for="privacy_policy in filterPrivacyPolicy" :key="privacy_policy.id" class="bg-white border-b hover:bg-gray-100">
                        <td class="w-4 p-4">
                            {{ privacy_policy.id }}
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <template v-if="!privacy_policy.is_published">
                            <router-link :to="{name:'edit_privacy_policy', params:{id: privacy_policy.id}}"
                        ...
                        <td class="px-6 py-4 text-center">
                            <router-link :to="{name:'preview_privacy_policy', params:{id: privacy_policy.id}}"
                                         class="text-blue-400 hover:text-blue-600">
                                <i class="fas fa-eye cursor-pointer mr-1"></i> preview </router-link>
                        </td>
                        <td v-if="privacy_policy.deleted_at === null" class="px-6 py-4">
                            <template v-if="privacy_policy.is_published">
                                <button @click="statusPrivacyPolicy(privacy_policy, false)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-circle-xmark cursor-pointer mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusPrivacyPolicy(privacy_policy, true)" class="px-8 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-globe cursor-pointer mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-else class="px-6 py-4">
                            <template v-if="privacy_policy.is_published" class="px-6 py-4 ">
                                <button @click="statusPrivacyPolicy(privacy_policy, false)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
                                    <i class="fas fa-circle-xmark text-gray-300 cursor-not-allowed mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusPrivacyPolicy(privacy_policy, true)" class="cursor-not-allowed text-gray-300 px-8 py-4" disabled>
                                    <i class="fas fa-globe text-gray-300 cursor-not-allowed mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-if="privacy_policy.deleted_at === null" class="px-6 py-4">
                            <button @click="destroyPrivacyPolicy(privacy_policy.id, true)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fa-solid fa-box-archive cursor-pointer mr-1"></i> Archive </button>
                        </td>
                        <td v-if="privacy_policy.deleted_at !== null" class="px-6 py-4">
                            <button @click="restorePrivacyPolicyMethod(privacy_policy.id)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fas fa-trash-arrow-up cursor-pointer mr-1"></i> Restore </button>
                        </td>
                        <td v-if="privacy_policy.deleted_at !== null" class="px-6 py-4">
                            <button @click="destroyPrivacyPolicy(privacy_policy.id, false)" class="px-6 py-4 text-red-400 hover:text-red-600 ">
                                <i class="fa-solid fa-trash-can cursor-pointer mr-1"></i> Delete </button>
                        </td>
                        <td v-else class="px-6 py-4">
                            <button @click="destroyPrivacyPolicy(privacy_policy.id)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
                                <i class="fa-solid fa-trash-can text-gray-300 cursor-not-allowed mr-1"></i> Delete </button>
                        </td>

                    </tr>
                </template>
                <template v-else-if="(!all_privacy_policy || all_privacy_policy.length <= 0) && !isLoading">
                    <tr>
            ...
            <div class="flex justify-center items-center mx-auto w-full">
                <section v-if="all_privacy_policy && all_privacy_policy.length !== 0 && meta.total > 5" class="py-2 w-full bg-white flex justify-center items-center mx-auto">
...
const { all_privacy_policy, restorePrivacyPolicy, publishPrivacyPolicy,fetchPrivacyPolicy, deletePrivacyPolicy, isLoading, meta } = usePrivacyPolicy();

onMounted(fetchPrivacyPolicy);

function paginate(pageNum){
    pageNumber.value = pageNum;
    fetchPrivacyPolicy(pageNum);
}

const filterPrivacyPolicy = computed(() => {
    return all_privacy_policy.value.filter((privacy_policy) => {
        return (
            privacy_policy.title
                .toLowerCase()
...
const destroyPrivacyPolicy = async (id, status) => {
...
const showPrivacyPolicy = async(id) => {
    console.log('preview coming soon...', id);
}

const restorePrivacyPolicyMethod = async (id) => {
    if(!window.confirm('Are You sure you want to restore this?')){
        return
    }
    await restorePrivacyPolicy(id);
    await fetchPrivacyPolicy();
}

const statusPrivacyPolicy = async (data,status) => {
    let my_status = status === true ? 'Publish' : 'UnPublish';

    if(!window.confirm('Are You sure you want to '+ my_status +' this?')){
        return
    }
    await publishPrivacyPolicy(data,status);
    await fetchPrivacyPolicy();
}
