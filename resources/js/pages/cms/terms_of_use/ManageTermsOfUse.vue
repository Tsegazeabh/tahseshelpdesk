<template>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 dark:text-gray-500">Terms Of Use</span>
                </div>
            </li>
        </ol>
    </nav>
    <div class="flex items-center justify-between">
        ...
        <router-link  :to="{name:'create_terms_of_use'}" class="text-sm float-right p-2 bg-green-400 text-white hover:bg-green-500 rounded-md">Create</router-link>
    </div>
...
<!--                    </th>-->
                ...
                <template v-if="all_terms_of_use && all_terms_of_use.length !== 0 && !isLoading">
                    <tr v-for="terms_of_use in filter_terms_of_use" :key="terms_of_use.id" class="bg-white border-b hover:bg-gray-100">
                        <td class="w-4 p-4">
                            {{ terms_of_use.id }}
                        </td>
...
<!--                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap" v-html="filteredDescription(terms_of_use.description)">-->
<!--                        </td>-->
                        <td class="px-6 py-4 text-red-400" :class="{'text-green-400': terms_of_use.is_published === 1}">
                            {{ terms_of_use.is_published === 1 ? 'Yes' : 'No' }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            <template v-if="!-terms_of_use.is_published">
                            <router-link :to="{name:'edit_terms_of_use', params:{id: terms_of_use.id}}"
                        ...
                        <td class="px-6 py-4 text-center">
                            <router-link :to="{name:'preview_terms_of_use', params:{id: terms_of_use.id}}"
                                         class="text-blue-400 hover:text-blue-600">
                                <i class="fas fa-eye cursor-pointer mr-1"></i> preview </router-link>
                        </td>
                        <td v-if="terms_of_use.deleted_at === null" class="px-6 py-4">
                            <template v-if="terms_of_use.is_published">
                                <button @click="statusTermsOfUse(terms_of_use, false)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-circle-xmark cursor-pointer mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusTermsOfUse(terms_of_use, true)" class="px-8 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-globe cursor-pointer mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-else class="px-6 py-4">
                            <template v-if="terms_of_use.is_published" class="px-6 py-4 ">
                                <button @click="statusTermsOfUse(terms_of_use, false)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
                                    <i class="fas fa-circle-xmark text-gray-300 cursor-not-allowed mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusTermsOfUse(terms_of_use, true)" class="cursor-not-allowed text-gray-300 px-8 py-4" disabled>
                                    <i class="fas fa-globe text-gray-300 cursor-not-allowed mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-if="terms_of_use.deleted_at === null" class="px-6 py-4">
                            <button @click="destroyTermsOfUse(terms_of_use.id)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fa-solid fa-box-archive cursor-pointer mr-1"></i> Archive </button>
                        </td>
                        <td v-if="terms_of_use.deleted_at !== null" class="px-6 py-4">
                            <button @click="restoreTermsOfUseMethod(terms_of_use.id)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fas fa-trash-arrow-up cursor-pointer mr-1"></i> Restore </button>
                        </td>
                        <td v-if="terms_of_use.deleted_at !== null" class="px-6 py-4">
                            <button @click="destroyTermsOfUse(terms_of_use.id)" class="px-6 py-4 text-red-400 hover:text-red-600 ">
                                <i class="fa-solid fa-trash-can cursor-pointer mr-1"></i> Delete </button>
                        </td>
                        <td v-else class="px-6 py-4">
                            <button @click="destroyTermsOfUse(terms_of_use.id)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
                                <i class="fa-solid fa-trash-can text-gray-300 cursor-not-allowed mr-1"></i> Delete </button>
                        </td>

                    </tr>
                </template>
                <template v-else-if="(!all_terms_of_use || all_terms_of_use.length) <= 0 && !isLoading">
                    <tr>
            ...
            <div class="flex justify-center items-center mx-auto w-full">
                <section v-if="all_terms_of_use && all_terms_of_use.length !== 0 && meta.total > 5" class="py-2 w-full bg-white flex justify-center items-center mx-auto">
...
import useTermsOfUse from "@composable/terms_of_use";

const search_key = ref('');
const pageNumber = ref(0);
const { all_terms_of_use, restoreTermsOfUse, publishTermsOfUse, fetchTermsOfUse, deleteTermsOfUse, isLoading, meta } = useTermsOfUse();

onMounted(fetchTermsOfUse);

function paginate(pageNum){
    pageNumber.value = pageNum;
    fetchTermsOfUse(pageNum);
}

const filter_terms_of_use = computed(() => {
    return all_terms_of_use.value.filter((terms_of_use) => {
        return (
            terms_of_use.title
                .toLowerCase()
