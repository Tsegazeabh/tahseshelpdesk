<template>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 dark:text-gray-500">Competencies</span>
                </div>
            </li>
        </ol>
    </nav>
    <div class="flex items-center justify-between">
        ...
        <router-link  :to="{name:'create_competency'}" class="text-sm float-right p-2 bg-green-400 text-white hover:bg-green-500 rounded-md">Create</router-link>
    </div>
    ...
    <div class="flex flex-col mt-6 w-full">
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <template v-if="allCompetency && allCompetency.length !== 0 && !isLoading">
                    <tr v-for="competency in filterCompetency" :key="competency.id" class="bg-white border-b hover:bg-gray-100">
                        <td class="w-4 p-4">
                            {{ competency.id }}
                        </td>
...
<!--                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap" v-html="filteredDescription(competency.description)">-->
<!--                        </td>-->
                        <td class="px-6 py-4 text-red-400" :class="{'text-green-400': competency.is_published === 1}">
                            {{ competency.is_published === 1 ? 'Yes' : 'No' }}
                        </td>
                        <td class="px-6 py-4 text-center">
                            <template v-if="!competency.is_published">
                            <router-link :to="{name:'edit_competency', params:{id: competency.id}}"
                        ...
                        <td class="px-6 py-4 text-center">
                            <router-link :to="{name:'preview_competency', params:{id: competency.id}}"
                                         class="text-blue-400 hover:text-blue-600">
                                <i class="fas fa-eye cursor-pointer mr-1"></i> preview </router-link>
                        </td>
                        <td v-if="competency.deleted_at === null" class="px-6 py-4">
                            <template v-if="competency.is_published">
                                <button @click="statusCompetency(competency, false)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-circle-xmark cursor-pointer mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusCompetency(competency, true)" class="px-8 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-globe cursor-pointer mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-else class="px-6 py-4">
                            <template v-if="competency.is_published" class="px-6 py-4 ">
                            ...
                            <template v-else class="px-6 py-4">
                                <button class="cursor-not-allowed text-gray-300 px-8 py-4" disabled>
                                    <i class="fas fa-globe text-gray-300 cursor-not-allowed mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-if="competency.deleted_at === null" class="px-6 py-4">
                            <button @click="destroyCompetency(competency.id, true)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fa-solid fa-box-archive cursor-pointer mr-1"></i> Archive </button>
                        </td>
                        <td v-if="competency.deleted_at !== null" class="px-6 py-4">
                            <button @click="restoreCompetencyMethod(competency.id)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fas fa-trash-arrow-up cursor-pointer mr-1"></i> Restore </button>
                        </td>
                        <td v-if="competency.deleted_at !== null" class="px-6 py-4">
                            <button @click="destroyCompetency(competency.id,false)" class="px-6 py-4 text-red-400 hover:text-red-600 ">
                ...
                <template v-else-if="(!allCompetency || allCompetency.length <= 0) && !isLoading">
                    <tr>
            ...
            <div class="flex justify-center items-center mx-auto w-full">
                <section v-if="allCompetency && allCompetency.length !== 0 && meta.total > 5" class="py-2 w-full bg-white flex justify-center items-center mx-auto">
...
import useCompetency from "@composable/competency";

const search_key = ref('');
const pageNumber = ref(0);
const { allCompetency, restoreCompetency, publishCompetency, fetchCompetency, deleteCompetency, isLoading, meta } = useCompetency();

onMounted(fetchCompetency);

function paginate(pageNum){
    pageNumber.value = pageNum;
    fetchCompetency(pageNum);
}

const filterCompetency = computed(() => {
    return allCompetency.value.filter((competency) => {
        return (
            competency.title
                .toLowerCase()
