        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 dark:text-gray-500">Contact Us Request</span>
                </div>
            </li>
        </ol>
    </nav>
...
<!--                        </th>-->
                    ...
                    <template v-if="allRequest && allRequest.length !== 0 && !isLoading">
                        <tr v-for="request in filterRequest" :key="request.id" class="bg-white border-b hover:bg-gray-100">
                            ...
                            <td v-else class="px-6 py-4">
                                <button @click="restoreRequestMethod(request.id)" class="px-6 py-4 text-green-400 hover:text-green-600 ">
                                    <i class="fas fa-trash-arrow-up cursor-pointer mr-1"></i> Open </button>
                            </td>
                            <td v-if="request.deleted_at === null" class="px-6 py-4 whitespace-nowrap">
                                    <router-link :to="{name:'view_request', params:{id: request.id}}"
                                                 class="text-blue-400 hover:text-blue-600">
                                        <i class="fas fa-eye cursor-pointer mr-1"></i>Read</router-link>
                            </td>
...
<!--                            </td>-->
                    ...
                    <template v-else-if="(!allRequest || allRequest.length <= 0) && !isLoading">
                        <tr>
                ...
                <div class="flex justify-center items-center mx-auto w-full">
                    <section v-if="allRequest && allRequest.length !== 0 && meta.total > 5" class="py-2 w-full bg-white flex justify-center items-center mx-auto">
...
import {computed, onMounted, ref} from 'vue';
    ...
    const filterRequest = computed(() => {
        return allRequest.value.filter((request) => {
            return (
                request.full_name
                    .toLowerCase()
...
const destroyRequest = async (id) => {
    if(!window.confirm('Are You Sure You want to close the request?')){
        return
    }
...
const restoreRequestMethod = async (id) => {
    if(!window.confirm('Are You sure you want to reopen request?')){
        return
    }
