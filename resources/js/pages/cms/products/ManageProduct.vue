<template>
    ...
    <div class="flex items-center justify-between">
        ...
        <router-link  :to="{name:'create_product'}" class="text-sm float-right p-2 bg-green-400 text-white hover:bg-green-500 rounded-md">Create</router-link>
    </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <template v-if="allProduct && allProduct.length !== 0 && !isLoading">
                    <tr v-for="product in filterProduct" :key="product.id" class="bg-white border-b hover:bg-gray-100">
                        <td class="w-4 p-4">
                            {{ product.id }}
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <template v-if="!product.is_published">
                        ...
                        <td class="px-6 py-4 text-center">
                            <router-link :to="{name:'preview_product', params:{id: product.id}}"
                                         class="text-blue-400 hover:text-blue-600">
                                <i class="fas fa-eye cursor-pointer mr-1"></i> preview </router-link>
                        </td>
                        <td v-if="product.deleted_at === null" class="px-6 py-4">
                            <template v-if="product.is_published">
                                <button @click="statusProduct(product, false)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-circle-xmark cursor-pointer mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusProduct(product, true)" class="px-8 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-globe cursor-pointer mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-else class="px-6 py-4">
                            <template v-if="product.is_published" class="px-6 py-4 ">
                                <button @click="statusProduct(product, false)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
                                    <i class="fas fa-circle-xmark text-gray-300 cursor-not-allowed mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusProduct(product, true)" class="cursor-not-allowed text-gray-300 px-8 py-4" disabled>
                                    <i class="fas fa-globe text-gray-300 cursor-not-allowed mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-if="product.deleted_at === null" class="px-6 py-4">
                            <button @click="destroyProduct(product.id, true)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fa-solid fa-box-archive cursor-pointer mr-1"></i> Archive </button>
                        </td>
                        <td v-if="product.deleted_at !== null" class="px-6 py-4">
                            <button @click="restoreProductMethod(product.id)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fas fa-trash-arrow-up cursor-pointer mr-1"></i> Restore </button>
                        </td>
                        <td v-if="product.deleted_at !== null" class="px-6 py-4">
                            <button @click="destroyProduct(product.id, false)" class="px-6 py-4 text-red-400 hover:text-red-600 ">
                                <i class="fa-solid fa-trash-can cursor-pointer mr-1"></i> Delete </button>
                        </td>
                        <td v-else class="px-6 py-4">
                            <button @click="destroyProduct(product.id)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
            ...
            <div class="flex justify-center items-center mx-auto w-full">
                <section v-if="allProduct && allProduct.length !== 0 && meta.total > 5" class="py-2 w-full bg-white flex justify-center items-center mx-auto">
...
const { allProduct, restoreProduct, publishProduct, fetchProduct, deleteProduct, isLoading, meta } = useProduct();
...
const destroyProduct = async (id, status) => {
...
const restoreProductMethod = async (id) => {
...
const statusProduct = async (data,status) => {
    let my_status = status === true ? 'Publish' : 'UnPublish';

    if(!window.confirm('Are You sure you want to '+ my_status +' this?')){
        return
    }
    await publishProduct(data,status);
    await fetchProduct();
}
