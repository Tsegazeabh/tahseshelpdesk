<template>
    ...
    <div class="flex items-center justify-between">
        ...
        <router-link  :to="{name:'create_service'}" class="text-sm float-right p-2 bg-green-400 text-white hover:bg-green-500 rounded-md">Create</router-link>
    </div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500">
                ...
                <template v-if="allService && allService.length !== 0 && !isLoading">
                    <tr v-for="service in filterService" :key="service.id" class="bg-white border-b hover:bg-gray-100">
                        <td class="w-4 p-4">
                            {{ service.id }}
                        </td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <template v-if="!-service.is_published">
                        ...
                        <td class="px-6 py-4 text-center">
                            <router-link :to="{name:'preview_service', params:{id: service.id}}"
                                         class="text-blue-400 hover:text-blue-600">
                                <i class="fas fa-eye cursor-pointer mr-1"></i> preview </router-link>
                        </td>
                        <td v-if="service.deleted_at === null" class="px-6 py-4">
                            <template v-if="service.is_published">
                                <button @click="statusService(service, false)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-circle-xmark cursor-pointer mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusService(service, true)" class="px-8 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-globe cursor-pointer mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-else class="px-6 py-4">
                            <template v-if="service.is_published" class="px-6 py-4 ">
                                <button @click="statusService(service, false)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
                                    <i class="fas fa-circle-xmark text-gray-300 cursor-not-allowed mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusService(service, true)" class="cursor-not-allowed text-gray-300 px-8 py-4" disabled>
                                    <i class="fas fa-globe text-gray-300 cursor-not-allowed mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-if="service.deleted_at === null" class="px-6 py-4">
                            <button @click="destroyService(service.id, true)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fa-solid fa-box-archive cursor-pointer mr-1"></i> Archive </button>
                        </td>
                        <td v-if="service.deleted_at !== null" class="px-6 py-4">
                            <button @click="restoreServiceMethod(service.id)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                <i class="fas fa-trash-arrow-up cursor-pointer mr-1"></i> Restore </button>
                        </td>
                        <td v-if="service.deleted_at !== null" class="px-6 py-4">
                            <button @click="destroyService(service.id, false)" class="px-6 py-4 text-red-400 hover:text-red-600 ">
                                <i class="fa-solid fa-trash-can cursor-pointer mr-1"></i> Delete </button>
                        </td>
                        <td v-else class="px-6 py-4">
                            <button @click="destroyService(service.id)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
                                <i class="fa-solid fa-trash-can text-gray-300 cursor-not-allowed mr-1"></i> Delete </button>
                        </td>
                    </tr>
                </template>
                <template v-else-if="(!allService || allService.length <= 0) && !isLoading">
                    <tr>
            ...
            <div class="flex justify-center items-center mx-auto w-full">
                <section v-if="allService && allService.length !== 0 && meta.total > 5" class="py-2 w-full bg-white flex justify-center items-center mx-auto">
...
const { allService, restoreService, publishService, fetchService, deleteService, isLoading, meta } = useService();

onMounted(fetchService);

function paginate(pageNum){
    pageNumber.value = pageNum;
    fetchService(pageNum);
}

const filterService = computed(() => {
    return allService.value.filter((service) => {
        return (
            service.title
                .toLowerCase()
...
const destroyService = async (id, status) => {
...
const restoreServiceMethod = async (id) => {
...
const statusService = async (data,status) => {
    let my_status = status === true ? 'Publish' : 'UnPublish';

    if(!window.confirm('Are You sure you want to '+ my_status +' this?')){
        return
    }
    await publishService(data,status);
    await fetchService();
}
