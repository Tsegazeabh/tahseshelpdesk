<template>
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2 dark:text-gray-500">Customers</span>
                </div>
            </li>
        </ol>
    </nav>
    <div class="flex items-center justify-between">
        ...
        <router-link  :to="{name:'create_customer'}" class="text-sm float-right p-2 bg-green-400 text-white hover:bg-green-500 rounded-md">Create</router-link>
    </div>
...
<!--                    </th>-->
                ...
                <tbody>
                <template v-if="allCustomer && allCustomer.length > 0">
                    <tr v-for="customer in filterCustomer" :key="customer.id" class="bg-white border-b hover:bg-gray-100">
                        <td class="w-4 p-4">
                            {{ customer.id }}
                        </td>
...
<!--                        </td>-->
                        ...
                        <td class="px-6 py-4 text-center">
                            <router-link :to="{name:'edit_customer', params:{id: customer.id}}" class="text-blue-400 hover:text-blue-600">
                                <i class="fas fa-edit cursor-pointer mr-1"></i> Edit</router-link>
                        </td>
                        <td class="px-6 py-4">
                            <button @click="showCustomer(customer.id)" class="text-blue-400 hover:text-blue-600">
                                <i class="fas fa-eye cursor-pointer mr-1"></i> preview </button>
                        </td>
                        <td v-if="customer.deleted_at === null" class="px-6 py-4">
                            <template v-if="customer.is_published">
                                <button @click="statusCustomer(customer, false)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-circle-xmark cursor-pointer mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusCustomer(customer, true)" class="px-8 py-4 text-blue-400 hover:text-blue-600 ">
                                    <i class="fas fa-globe cursor-pointer mr-1"></i> publish </button>
                            </template>
                        </td>
                        <td v-else class="px-6 py-4">
                            <template v-if="customer.is_published" class="px-6 py-4 ">
                                <button @click="statusCustomer(customer, false)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
                                    <i class="fas fa-circle-xmark text-gray-300 cursor-not-allowed mr-1"></i> Unpublish </button>
                            </template>
                            <template v-else class="px-6 py-4">
                                <button @click="statusCustomer(customer, true)" class="cursor-not-allowed text-gray-300 px-8 py-4" disabled>
                        ...
                        <td v-if="customer.deleted_at !== null" class="px-6 py-4">
                            <button @click="restoreCustomerMethod(customer.id)" class="px-6 py-4 text-blue-400 hover:text-blue-600 ">
                        ...
                        <td v-else class="px-6 py-4">
                            <button @click="destroyCustomer(customer.id)" class="cursor-not-allowed text-gray-300 px-6 py-4" disabled>
...
const { allCustomer, publishCustomer, fetchCustomer, updateCustomer, deleteCustomer, restoreCustomer, notification } = useCustomer();
...
const restoreCustomerMethod = async (id) => {
...
const statusCustomer = async (data,status) => {
    if(!window.confirm('Are You sure you want to publish this?')){
        return
    }
    await publishCustomer(data,status);
    await fetchCustomer();
}
</script>
