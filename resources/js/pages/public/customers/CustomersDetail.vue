    ...
    <home-layout>
        <div class="grid grid-cols-3">
            <div class="col-span-3 md:col-span-2 py-16 px-16">
                <h2 class="capitalize my-2 text-gray-800 text-md font-semibold my-0">{{customer.title}}</h2>
                <p class="text-gray-400 text-sm my-0">{{getDate(customer.published_at) }}</p>
                <div class="mt-3 ">
                    <div class="text-gray-700 description" v-html="customer.description">
                    </div>
                </div>
            </div>
            <div class="col-span-3 md:col-span-1 pt-16 px-4 bg-gray-100 mb-0 pb-0">
                ...
                <div v-if="allCustomer && allCustomer.length !== 0 && !isLoading" class="overflow-x-hidden overflow-y-auto space-y-4 h-[70vh] w-full bg-gray-200 rounded-md p-2">
                    <template v-for="customers in allCustomer" :key="customers.id">
                        <button v-if="customers.id !== customer.id" @click="navigate(customers)" class="flex bg-white h-32 overflow-hidden rounded-md hover:border hover:border-primary">
                            <div class="h-full w-1/4">
                                <img :src="getFirstImage(customers.description)" alt="image" class="w-full h-full object-cover">
                            </div>
                            <div class="p-2 w-3/4 h-full flex flex-col justify-start text-left">
                                <div>
                                    <h3 class="text-sm lg:text-lg text-gray-700 font-normal my-1 uppercase ">{{ getTitleShortened(customers.title) }}</h3>
                                    <p class="text-xs lg:text-base text-gray-500 capitalize leading-tight">{{getDescriptionShort(customers.description)}}</p>
                                </div>
                                <div class="flex justify-start items-end w-full h-full">
                                    <p class="text-xs text-primary capitalize"><i class="mr-2 fas fa-calendar-alt"></i>{{getDate(customers.published_at)}}</p>
                                </div>
                            </div>
                        </button>
                    </template>
...
onMounted(getCustomer(props.id),fetchCustomer());

function navigate(customers){
    customer.value = customers;
    router.push({name: 'customers.detail', params: {id: customers.id}});
}
