    ...
    <home-layout>
        <div class="flex flex-col justify-center items-center p-6 bg-gray-100 overflow-x-hidden">
            ...
            <template v-if="allCompetency && allCompetency.length !== 0 && !isLoading">
                <h2 class="text-center font-bold p-4 m-2 uppercase text-xl font-normal tracking-wide">Our Core Competencies</h2>
                ...
                <div class="flex flex-col sm:flex-row flex-wrap justify-center items-center p-6 space-y-16 sm:space-y-4 h-full">
                    <div v-for="competency in filterCompetency" :key="competency.id" class="mt-4 overflow-hidden flex flex-col md:flex-row h-96 w-64 md:w-[40vw] md:h-72 bg-gray-200 rounded-md shadow-md shadow-gray-600 mr-4">
...
const { allCompetency, fetchCompetency, isLoading, meta } = usePublicCompetency();
