<template>
    <home-layout>
        <div class="grid grid-cols-3">
            <div v-for="competenc in competency" :key="competenc.id" class="col-span-3 md:col-span-2 py-16 px-16">
                <h2 class="capitalize my-2 text-gray-800 text-md font-semibold my-0">{{competenc.title}}</h2>
                <p class="text-gray-400 text-sm my-0">{{getDate(competenc.published_at) }}</p>
                <div class="mt-3 ">
                    <div class="text-gray-700 description" v-html="competenc.description">
                    </div>
                </div>
            </div>
...
const { competency, previewCompetency } = usePublicCompetency();
const { getDate } = helpers();
const props = defineProps(['id']);

onMounted(()=>{
    previewCompetency(props.id)
});
