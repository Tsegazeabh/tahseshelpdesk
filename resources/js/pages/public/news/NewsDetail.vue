    ...
    <home-layout>
        <div class="grid grid-cols-3">
            ...
            <div class="col-span-3 md:col-span-1 pt-16 px-4 bg-gray-100 mb-0 pb-0">
                ...
                <div v-if="allNews && allNews.length !== 0" class="overflow-x-hidden overflow-y-auto space-y-4 h-[70vh] w-full bg-gray-200 rounded-md p-2">
                    <template v-for="news_all in allNews" :key="news_all.id">
                        <button v-if="news_all.id !== news.id" @click="navigate(news_all)" class="flex bg-white h-32 overflow-hidden rounded-md">
                            ...
                            <div class="p-2 w-3/4 h-full flex flex-col justify-start text-left">
                                <div>
                                    <h3 class="text-sm lg:text-lg text-gray-700 font-normal my-1 uppercase ">{{ getTitleShortened(news_all.title) }}</h3>
                                    <p class="text-xs lg:text-base text-gray-500 capitalize leading-tight">{{getDescriptionShort(news_all.description)}}</p>
                                </div>
                                <div class="flex justify-start items-end w-full h-full">
                                    <p class="text-xs text-primary capitalize"><i class="mr-2 fas fa-calendar-alt"></i>{{getDate(news_all.published_at)}}</p>
                                </div>
                            </div>
                        </button>
                    </template>
...
const { news, allNews, getNews, fetchNews } = usePublicNews();
const { getDate, getFirstImage, getTitleShortened, getDescriptionShort, getDescriptionShortened } = helpers();
const router = useRouter();

const props = defineProps(['id']);

onMounted(getNews(props.id),fetchNews());

function navigate(news_all){
    news.value = news_all;
    router.push({name: 'news_all.detail', params: {id: news_all.id}});
}
