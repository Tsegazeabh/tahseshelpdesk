<template>
    <home-layout>
        <div class="flex flex-col justify-center items-center p-6 bg-gray-100 overflow-x-hidden">
            ...
            <template v-if="allNews && allNews.length !== 0 && !isLoading">
                <h2 class="text-center font-bold p-4 m-2 uppercase text-xl font-normal tracking-wide">Our News</h2>
                ...
                <div class="flex flex-col sm:flex-row flex-wrap justify-center items-center p-6 space-y-16 sm:space-y-4 h-full">
                    <div v-for="news in filterNews" :key="news.id" class="mt-4 overflow-hidden flex flex-col md:flex-row h-96 w-64 md:w-[40vw] md:h-72 bg-gray-200 rounded-md shadow-md shadow-gray-600 mr-4">
                        <div class="w-full h-2/5 md:w-1/3 md:h-full border-gray-200 md:border-r-8 bg-gray-300">
                            <img class="w-full h-full rounded-t-md md:rounded-l-md md:rounded-r-none object-cover" :src="getFirstImage(news.description)" alt="image name">
                        </div>
                        <div class="flex flex-col justify-between items-start h-3/5 md:h-full w-full md:w-2/3 p-4 bg-white">
                            <div class="w-full">
                                <h3 class="font-normal capitalize text-base lg:text-lg text-gray-800">{{getTitleShortened(news.title)}}</h3>
                                <p class="text-wrap flex-wrap text-sm md:text-base py-3 text-gray-500 capitalize">{{getDescriptionShortened(news.description)}}</p>
                            </div>
                            <div class="flex justify-between items-center py-1 w-full">
                                <div>
                                    <p class="text-gray-400 text-xs md:text-sm py-2"><i class="text-xs fas fa-calendar"></i> {{news.published_at}}</p>
                                </div>
                                <div>
                                    <router-link :to="{name:'news.detail', params:{id: news.id}}" class="py-1 px-2 lg:py-2 lg:px-4 rounded-md text-white hover:bg-primary bg-primary/90 hover:text-white text-white font-normal float-right text-sm md:text-md lg:text-base">View</router-link>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="flex justify-center items-center mx-auto w-full">
                        <section v-if="allNews && allNews.length !== 0 && meta.total > 5" class="py-2 w-full bg-white flex justify-center items-center mx-auto">
            ...
            <template v-else-if="(!allNews || allNews.length <= 0) && !isLoading">
...
const { allNews, fetchNews, isLoading, meta } = usePublicNews();
