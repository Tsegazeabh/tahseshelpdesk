<template>
    <template v-if="latest_product && latest_product.length !== 0">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 gap-x-4 py-6">
            <div v-for="product in latest_product" :key="product.id" class="mx-auto w-[300px] h-[350px] md:h-[400px] grid grid-rows-3 grid-cols-1 overflow-hidden shadow-xl shadow-gray-600 rounded-fine border border-[#00cba9] border-1">
...
<script setup>
    ...
    const { latestProduct, latest_product} = usePublicProduct();
