    ...
    <home-layout>
        <div class="flex flex-col justify-center items-center p-6 bg-gray-100 overflow-x-hidden">
            ...
            <div class="flex flex-col sm:flex-row flex-wrap justify-center items-center p-6 space-y-16 sm:space-y-4 h-full">
                <div v-for="product in filterProduct" :key="product.id" class="mt-4 overflow-hidden flex flex-col md:flex-row h-96 w-64 md:w-[40vw] md:h-72 bg-gray-200 rounded-md shadow-md shadow-gray-600 mr-4">
                    <div class="w-full h-2/5 md:w-1/3 md:h-full border-gray-200 md:border-r-8 bg-gray-300">
                        <img class="w-full h-full rounded-t-md md:rounded-l-md md:rounded-r-none object-cover" :src="getFirstImage(product.description)" alt="image name">
                    </div>
                    <div class="flex flex-col justify-between items-start h-3/5 md:h-full w-full md:w-2/3 p-4 bg-white">
                        <div class="w-full">
                            <h3 class="font-normal capitalize text-base lg:text-lg text-gray-800">{{getTitleShortened(product.title)}}</h3>
                            <p class="text-wrap flex-wrap text-sm md:text-base py-3 text-gray-500 capitalize">{{getDescriptionShortened(product.description)}}</p>
                        </div>
                        <div class="flex justify-between items-center py-1 w-full">
                            <div>
                                <p class="text-gray-400 text-xs md:text-sm py-2"><i class="text-xs fas fa-calendar"></i> {{product.published_at}}</p>
                            </div>
                            <div>
                                <router-link :to="{name:'products.detail', params:{id: product.id}}" class="py-1 px-2 lg:py-2 lg:px-4 rounded-md text-white hover:bg-primary bg-primary/90 hover:text-white text-white font-normal float-right text-sm md:text-md lg:text-base">View</router-link>
                            </div>
                        </div>
                    </div>
                </div>
...
const { allProduct, fetchProduct, isLoading, meta } = usePublicProduct();
