    ...
    <home-layout>
        <div class="grid grid-cols-3">
            <div class="col-span-3 md:col-span-2 py-16 px-16">
                <h2 class="capitalize my-2 text-gray-700 text-md font-semibold my-0">{{service.title}}</h2>
                <p class="text-gray-400 text-sm my-0">{{getDate(service.published_at) }}</p>
                <div class="mt-3 ">
                    <div class="text-gray-500 description" v-html="service.description">
                    </div>
                </div>
            </div>
            <div class="col-span-3 md:col-span-1 pt-16 px-4 bg-gray-100 mb-0 pb-0">
                ...
                <div v-if="allService && allService.length !== 0" class="overflow-x-hidden overflow-y-auto space-y-4 h-[70vh] w-full bg-gray-200 rounded-md p-2">
                    <template v-for="services in allService" :key="services.id">
                        <button v-if="services.id !== service.id" @click="navigate(services)" class="flex bg-white h-32 overflow-hidden rounded-md">
...
const { service, allService, getService, fetchService } = usePublicService();
